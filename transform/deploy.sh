...
STACK_NAME="${STACK_NAME:-recordings-transform-lambda}"
S3_PREFIX="${S3_PREFIX:-recordings/transform}"
...
case "${stack_status}" in
  ROLLBACK_FAILED|ROLLBACK_COMPLETE|CREATE_FAILED|DELETE_FAILED)
...
sam deploy "${deploy_args[@]}" \
  --parameter-overrides "SharedSecret=${RECORDINGS_TRANSFORM_SHARED_SECRET}"
